...
- 2026-07-08 — Infinity wake, ninth pass — birth tuning (user: motion/density/spread/lifetime all good; each wavelet should START smaller + more opaque + already out to the side/front, and FINISH in the same state as v8). Kept the final state identical and changed only birth: half-length `INIT_LEN 0.10→0.05` + `GROW 0.24→0.29` (final 0.34 unchanged); dab radius no longer fixed — now `= half·WAKELET_DAB_RATIO 0.34` so the WHOLE wavelet scales (final ≈ old 0.11 lw), making "half size" actually read; opacity envelope `min(1,t/0.12)·(1-t) → min(1,t/0.04)·(1-t)` (near-instant fade-in → young wavelets ~2× more opaque, identical for t≥0.12 so mid/final unchanged); starting side offset `INIT_OFF 0.10→0.24` + `SPREAD 0.52→0.38` (final 0.62 unchanged) so small waves start out to the side, not under the touch; new `WAKELET_FRONT_OFF_LW 0.18` shifts birth ahead of the shed point (toward the front). `npm run build` clean. **On-device confirm pending.**
